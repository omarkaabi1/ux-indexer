...
exports.makeIndexName = makeIndexName;
exports.generateOrmModels = generateOrmModels;
exports.resolveTableNames = resolveTableNames;
...
const log = (0, logger_1.createLogger)('sqd:typeorm-codegen');
/**
 ...
 */
function makeIndexName(entity, fields, unique) {
    const identity = `${entity}|${fields.join(',')}|${unique ? 'unique' : ''}`;
    const hash = (0, crypto_1.createHash)('sha1').update(identity).digest('hex').slice(0, INDEX_NAME_HASH_LENGTH);
    const readable = ['idx', (0, util_naming_1.toSnakeCase)(entity), ...fields.map(util_naming_1.toSnakeCase)].join('_');
...
function generateOrmModels(model, dir) {
    ...
    const indexNamesByName = new Map();
    function indexNameFor(entity, fields, unique) {
        const name = makeIndexName(entity, fields, unique);
        const identity = `${entity}(${fields.join(', ')})${unique ? ' unique' : ''}`;
        const previous = indexNamesByName.get(name);
        if (previous != null && previous !== identity) {
            throw new Error(`Index name "${name}" is generated for two different indexes — ${previous} and ` +
    ...
    function generateEntity(name, entity) {
        ...
        const nameIndex = (fields, unique) => indexNameFor(name, fields, unique);
        ...
        if (tableName === (0, util_naming_1.toSnakeCase)(name)) {
            out.line('@Entity_()');
        }
        else {
                ...
                switch (prop.type.kind) {
                    ...
                    case 'fk': {
                        ...
                        if (getFieldIndex(entity, key)?.unique) {
                            ...
                            out.line(`@OneToOne_(() => ${prop.type.entity}, {nullable: true${fkOptions}})`);
                            ...
                            out.line(`@ManyToOne_(() => ${prop.type.entity}, {nullable: true${fkOptions}})`);
                        }
                        break;
                    }
                    case 'lookup':
 ...
 */
function resolveTableNames(model) {
    const tableNames = new Map();
    const owners = new Map();
    for (const name in model) {
        if (model[name].kind !== 'entity')
            continue;
        const fullName = (0, util_naming_1.toSnakeCase)(name);
        ...
        if (truncated) {
            ...
            log.warn(`Table name "${fullName}" (entity "${name}") exceeds the PostgreSQL identifier ` +
        ...
        if (owner != null) {
            ...
            throw new Error(`Entities "${owner.name}" and "${name}" both map to table name "${tableName}"` +
...
function addIndexAnnotation(entity, field, imports, out, nameIndex) {
