...
const LEGACY_HOT_BLOCK_REPAIR = `DELETE FROM ${SCHEMA}.hot_block ` +
...
describe('TypeormDatabase — orphan repair on a deep hot window', function () {
    (0, util_1.useDatabase)(['CREATE TABLE item (id text primary key, name text)']);
    let db;
    let cockroach = false;
    beforeAll(async () => {
        cockroach = await (0, util_1.isCockroach)();
    });
    beforeEach(() => {
    ...
    afterEach(async () => {
        ...
        await (0, util_1.withClient)(async (client) => {
            await client.query(`ALTER DATABASE "${util_1.db_config.database}" RESET work_mem`);
            await client.query(`ALTER DATABASE "${util_1.db_config.database}" RESET statement_timeout`);
        }).catch(() => { });
    });
    /**
     ...
     */
    async function seedHotWindow(client, blocks = HOT_BLOCKS) {
        await client.query(`INSERT INTO ${SCHEMA}.hot_block (height, hash)
             SELECT g, 'h-' || g FROM generate_series(1, $1::int) AS g`, [blocks]);
        await client.query(`INSERT INTO ${SCHEMA}.hot_change_log (block_height, index, change)
             SELECT b.height, i, '{"kind":"sentinel"}'::jsonb
               FROM generate_series(1, $1::int) AS b(height),
                    generate_series(0, $2::int) AS i`, [blocks, CHANGES_PER_BLOCK - 1]);
    ...
    async function explainJson(client, sql) {
    ...
    async function explain(client, sql) {
        return planTags(await explainJson(client, sql));
    }
    async function totalCost(client, sql) {
    ...
    it('plans every repair statement as an anti-join, even under a tiny work_mem', async function (ctx) {
        ...
        await (0, util_1.withClient)(async (client) => {
            ...
            for (let sql of (0, repair_1.orphanRepairStatements)(SCHEMA)) {
    ...
    it('is orders of magnitude cheaper than the legacy NOT IN formulation', async function (ctx) {
        ...
        await (0, util_1.withClient)(async (client) => {
            ...
            let fixedCost = await totalCost(client, (0, repair_1.orphanRepairStatements)(SCHEMA)[1]);
    ...
    it('connect() returns the full hot chain on a deep hot window at small work_mem', async function (ctx) {
        ...
        await (0, util_1.withClient)(async (client) => {
            ...
            await client.query(`ALTER DATABASE "${util_1.db_config.database}" SET work_mem = '${SMALL_WORK_MEM}'`);
            await client.query(`ALTER DATABASE "${util_1.db_config.database}" SET statement_timeout = '20s'`);
        });
        db = new database_1.TypeormDatabase({ projectDir: __dirname, supportHotBlocks: true });
    ...
    it('removes exactly the orphaned rows and keeps the healthy hot window', async function () {
        ...
        await (0, util_1.withClient)(async (client) => {
            ...
            let fk = (await client.query(`SELECT conname FROM pg_constraint
                      WHERE conrelid = '${SCHEMA}.hot_change_log'::regclass AND contype = 'f' LIMIT 1`)).rows[0].conname;
            ...
            await client.query(`INSERT INTO ${SCHEMA}.template_registry (key, value, type, block_number, height)
        ...
        const count = async (sql) => Number((await em.query(sql))[0].n);
        ...
        expect(await count(`SELECT COUNT(*)::int AS n FROM ${SCHEMA}.hot_change_log`)).toBe(blocks * CHANGES_PER_BLOCK);
        expect(await count(`SELECT COUNT(*)::int AS n FROM ${SCHEMA}.template_registry WHERE key = 'keep'`)).toBe(1);
    });
});
//# sourceMappingURL=orphan-repair.test.js.map