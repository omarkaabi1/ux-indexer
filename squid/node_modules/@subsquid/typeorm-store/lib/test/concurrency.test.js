...
describe('TypeormDatabase — concurrent writers on the same schema', function () {
    ...
    let db2;
    beforeEach(() => {
        db1 = new database_1.TypeormDatabase({ projectDir: __dirname, supportHotBlocks: true });
        db2 = new database_1.TypeormDatabase({ projectDir: __dirname, supportHotBlocks: true });
    });
    afterEach(async () => {
    ...
    it('sequential transact from a stale instance: second fails with RACE_MSG (prevHead path)', async () => {
        ...
        let error;
        try {
            await db2.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 20, hash: 'h-20' } }, async () => { });
        }
        catch (e) {
            error = e;
        }
    ...
    it('nonce-WHERE guard fires when both writers read the same state and pass prevHead (READ COMMITTED)', async function () {
        ...
        const readCommittedDb1 = new database_1.TypeormDatabase({
            projectDir: __dirname,
            supportHotBlocks: true,
            isolationLevel: 'READ COMMITTED',
        });
        const readCommittedDb2 = new database_1.TypeormDatabase({
            projectDir: __dirname,
            supportHotBlocks: true,
            isolationLevel: 'READ COMMITTED',
        });
        try {
            ...
            await readCommittedDb1.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 5, hash: 'h-5' } }, async () => { });
            ...
            let error;
            try {
                await readCommittedDb2.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 5, hash: 'h-5-alt' } }, async () => { });
            }
            catch (e) {
                error = e;
            }
    ...
    it('propagates 40001 past the retry budget when the handler keeps failing', async function () {
        ...
        await expect(db1.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 1, hash: 'h-1' } }, async () => {
            attempts += 1;
            throw error;
        })).rejects.toBe(error);
    ...
    it('simultaneous transact from two instances: exactly one wins, the other gets RACE_MSG', async () => {
        ...
        // timing).
        const p1 = db1.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 10, hash: 'h-10' } }, async () => { });
        const p2 = db2.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 20, hash: 'h-20' } }, async () => { });
