...
describe('TypeormDatabase — crash recovery', function () {
        ...
        it('rolls back every write from every block group, not just the one that threw', async function () {
            ...
            await db.transactHot2({
                ...
                for (let i = beg; i < end; i++) {
                    await store.insert(new model_1.Data({ id: `baseline-${i}`, text: 'baseline' }));
                }
            });
            ...
            async function snapshot() {
                ...
                const hotBlocks = await em.query('SELECT height, hash FROM squid_processor.hot_block ORDER BY height');
                const count = (await em.query('SELECT COUNT(*)::int AS n FROM squid_processor.hot_change_log'))[0].n;
                return {
                    status: status.map((r) => ({
                        height: Number(r.height),
                        hash: r.hash,
                    })),
                    hotBlocks: hotBlocks.map((r) => ({
                        height: Number(r.height),
                        hash: r.hash,
                    })),
            ...
            try {
                await db.transactHot2({
                    ...
                    for (let i = beg; i < end; i++) {
                        ...
                        await store.insert(new model_1.Data({ id: `in-flight-${block.height}`, text: 'about to die' }));
                    }
                    if (callCount >= 2) {
    ...
    describe('connect() with corrupted internal state', () => {
        it('handles hot blocks where the user handler made no writes', async function () {
            ...
            const em = await (0, util_1.getEntityManager)();
            expect(Number((await em.query('SELECT COUNT(*)::int AS n FROM squid_processor.hot_block'))[0].n)).toBe(3);
            expect(Number((await em.query('SELECT COUNT(*)::int AS n FROM squid_processor.hot_change_log'))[0].n)).toBe(3);
        ...
        it('removes orphan template_registry rows whose height matches no state', async function () {
            ...
            const em = await (0, util_1.getEntityManager)();
            await em.query(`INSERT INTO squid_processor.template_registry (key, value, type, block_number, height)
            ...
            db = new database_1.TypeormDatabase({ projectDir: __dirname, supportHotBlocks: true });
            await db.connect();
            const remaining = (await em.query('SELECT COUNT(*)::int AS n FROM squid_processor.template_registry WHERE height = 999'))[0].n;
        ...
        it('detects or repairs an orphan hot_block row (hot_block without change-log entries)', async function () {
            ...
            const em = await (0, util_1.getEntityManager)();
            await em.query(`INSERT INTO squid_processor.hot_block (height, hash) VALUES (42, 'h-42-orphan')`);
            await db.disconnect();
            db = new database_1.TypeormDatabase({ projectDir: __dirname, supportHotBlocks: true });
            await db.connect();
            ...
            const surviving = (await em.query('SELECT COUNT(*)::int AS n FROM squid_processor.hot_block WHERE height = 42'))[0].n;
        ...
        it('removes orphan hot_change_log rows whose block_height matches no hot_block', async function () {
            ...
            const fkName = (await em.query(`SELECT conname FROM pg_constraint
            ...
            await em.query(`INSERT INTO squid_processor.hot_change_log (block_height, index, change)
                 VALUES (99, 0, '{"kind":"insert","table":"item","id":"ghost"}'::jsonb)`);
            // Reconnect — this is where connect() should notice and repair.
            await db.disconnect();
            db = new database_1.TypeormDatabase({ projectDir: __dirname, supportHotBlocks: true });
            await db.connect();
            const remaining = (await em.query('SELECT COUNT(*)::int AS n FROM squid_processor.hot_change_log WHERE block_height = 99'))[0].n;
            expect(Number(remaining)).toBe(0);
        });
    });
});
