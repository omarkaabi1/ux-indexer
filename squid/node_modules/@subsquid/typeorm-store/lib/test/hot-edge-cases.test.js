...
describe('TypeormDatabase — hot-path edge cases', function () {
    ...
    it('rollbackBlock restores an id-only entity after remove', async function () {
        ...
        await db.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 0, hash: 'h-0' } }, async (store) => {
            await store.insert(new model_1.IdOnly('x'));
        });
        const em = await (0, util_1.getEntityManager)();
        expect(await em.count(model_1.IdOnly)).toBe(1);
        ...
        await db.transactHot({
            ...
            await store.remove(model_1.IdOnly, 'x');
        });
        expect(await em.count(model_1.IdOnly)).toBe(0);
        ...
        expect(await em.count(model_1.IdOnly)).toBe(1);
        const restored = await em.findOneOrFail(model_1.IdOnly, { where: { id: 'x' } });
    ...
    it('rollbackBlock restores a non-id entity field after hot-block update', async function () {
        ...
        await db.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 0, hash: 'h-0' } }, async (store) => {
            await store.insert(new model_1.Item('i1', 'initial'));
        });
        const em = await (0, util_1.getEntityManager)();
        expect((await em.findOneOrFail(model_1.Item, { where: { id: 'i1' } })).name).toBe('initial');
        ...
        await db.transactHot({
            ...
            const item = await store.findOneOrFail(model_1.Item, { where: { id: 'i1' } });
            item.name = 'changed';
            await store.save(item);
        });
        expect((await em.findOneOrFail(model_1.Item, { where: { id: 'i1' } })).name).toBe('changed');
...
//# sourceMappingURL=hot-edge-cases.test.js.map