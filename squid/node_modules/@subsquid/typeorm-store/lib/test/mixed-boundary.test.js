...
describe('TypeormDatabase — finality boundary', function () {
    ...
    it('finality-only update (empty newBlocks, baseHead at tip) advances status and purges newly-finalized hot_blocks', async function () {
        ...
        let hotHeights = (await em.query('SELECT height FROM squid_processor.hot_block ORDER BY height')).map((r) => r.height);
        ...
        hotHeights = (await em.query('SELECT height FROM squid_processor.hot_block ORDER BY height')).map((r) => r.height);
    ...
    it('mixed finalized+hot batch: finalized blocks have no change_log, hot blocks are rollback-addressable', async function () {
        ...
        await db.transactHot2({
            ...
            for (let i = beg; i < end; i++) {
                const height = 5 + i;
                await store.insert(new model_1.Data({ id: `row-${height}`, text: `row-${height}` }));
            }
        });
        const em = await (0, util_1.getEntityManager)();
        // User table has all five rows — the callback ran for every block.
        const ids = (await em.find(model_1.Data, { order: { id: 'asc' } })).map((r) => r.id);
        ...
        const hotBlocks = (await em.query('SELECT height FROM squid_processor.hot_block ORDER BY height')).map((r) => r.height);
        ...
        const changeLogHeights = (await em.query('SELECT DISTINCT block_height FROM squid_processor.hot_change_log ORDER BY block_height')).map((r) => r.block_height);
        ...
        await db.transactHot2({
            ...
            for (let i = beg; i < end; i++) {
                await store.insert(new model_1.Data({ id: 'row-8-alt', text: 'row-8-alt' }));
            }
        });
        const postReorgIds = (await em.find(model_1.Data, { order: { id: 'asc' } })).map((r) => r.id);
        ...
        const postHot = (await em.query('SELECT height, hash FROM squid_processor.hot_block ORDER BY height')).map((r) => ({ height: Number(r.height), hash: r.hash }));
    ...
    it('transact unconditionally clears hot_block; transactHot2 only clears rows ≤ finalizedHead', async function () {
        ...
        async function hotBlockHeights() {
            const rows = await em.query('SELECT height FROM squid_processor.hot_block ORDER BY height');
        ...
        await db.transact({ prevHead: { height: 3, hash: 'h-3' }, nextHead: { height: 10, hash: 'h-10' } }, async () => { });
        expect(await hotBlockHeights()).toEqual([]);
    });
});
//# sourceMappingURL=mixed-boundary.test.js.map