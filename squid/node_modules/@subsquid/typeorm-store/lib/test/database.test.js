    ...
    it('.transactHot() flow', async function () {
        ...
        await db.transactHot({
            ...
            switch (block.height) {
                ...
                case 2:
                    expect(await store.get(model_1.Data, '1')).toEqual(a1);
                    await store.insert([a2, a3]);
            }
        });
        expect(await em.find(model_1.Data, { order: { id: 'asc' } })).toEqual([
            a1, a2, a3
        ]);
        ...
        await db.transactHot({
            ...
            expect(await store.find(model_1.Data)).toEqual([a1]);
            await store.remove(a1);
        });
        expect(await em.find(model_1.Data)).toEqual([]);
        await db.transactHot({
            finalizedHead: { height: 0, hash: '0' },
            baseHead: { height: 1, hash: 'a-1' },
            newBlocks: [
        ...
        expect(await em.find(model_1.Data)).toEqual([a1]);
        await db.disconnect();
        expect(await db.connect()).toMatchObject({
            height: 0,
            hash: '0',
