...
describe('TypeormDatabase — init/DDL smoke test on the active backend', function () {
    ...
    it('connect() succeeds on a fresh schema', async function () {
        ...
        const tables = (await em.query(`SELECT table_name FROM information_schema.tables
    ...
    it('a full commit cycle (transact + transactHot2 + rollback) runs end-to-end', async function () {
        ...
        await db.transact({
            ...
            await store.insert(new model_1.Data({ id: 'finalized-1', text: 'committed' }));
        });
        ...
        await db.transactHot2({
            ...
            await store.insert(new model_1.Data({ id: 'hot-1', text: 'unfinalized' }));
        });
        ...
        await db.transactHot2({
            ...
            await store.insert(new model_1.Data({ id: 'hot-alt', text: 'post-reorg' }));
        });
        ...
        const em = await (0, util_1.getEntityManager)();
        const rows = (await em.query('SELECT id FROM "data" ORDER BY id')).map((r) => r.id);
        expect(rows).toEqual(['finalized-1', 'hot-alt']);
    });
});
//# sourceMappingURL=cockroach-smoke.test.js.map