...
describe('TypeormDatabase — transactHot vs transactHot2 parity', function () {
    ...
    async function workOnBlock(store, block) {
        await store.insert(new model_1.Data({ id: `b-${block.height}`, text: block.hash, integer: block.height }));
    }
    const callHot = (db, info) => db.transactHot(info, async (store, block) => {
        await workOnBlock(store, block);
    });
    const callHot2 = (db, info) => db.transactHot2(info, async (store, sliceBeg, sliceEnd) => {
        for (let i = sliceBeg; i < sliceEnd; i++) {
            await workOnBlock(store, info.newBlocks[i]);
        }
    });
    async function runFullSequence(apply) {
    ...
    async function snapshot() {
        ...
        return {
            ...
            changeLog: await em.query('SELECT block_height, index FROM squid_processor.hot_change_log ORDER BY block_height, index'),
            data: await em.query('SELECT id, "text", "integer" FROM "data" ORDER BY id'),
        };
    }
    async function reset() {
        await (0, util_1.databaseDelete)();
        await (0, util_1.databaseInit)(SCHEMA);
    }
...
//# sourceMappingURL=transact-parity.test.js.map