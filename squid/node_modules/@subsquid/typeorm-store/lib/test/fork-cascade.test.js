...
describe('TypeormDatabase — cascading reorgs', function () {
    ...
    it('two forks back-to-back leave only the latest branch behind', async function () {
        ...
        await db.transactHot({
            ...
            await store.insert(new model_1.Data({ id: `main-${block.height}`, text: `main-${block.height}` }));
        });
        ...
        await db.transactHot({
            ...
            await store.insert(new model_1.Data({ id: `fork-a-${block.height}`, text: `fork-a-${block.height}` }));
        });
        const em = await (0, util_1.getEntityManager)();
        // Intermediate check: main-1, main-2, main-3, fork-a-4, fork-a-5, fork-a-6.
        const afterA = (await em.find(model_1.Data, { order: { id: 'asc' } })).map((r) => r.id);
        ...
        await db.transactHot({
            ...
            await store.insert(new model_1.Data({ id: `fork-b-${block.height}`, text: `fork-b-${block.height}` }));
        });
        // Final: only main-1..2 and fork-b-3..7 remain.
        const afterB = (await em.find(model_1.Data, { order: { id: 'asc' } })).map((r) => r.id);
        ...
        expect(await em.count(model_1.Data, { where: { id: 'fork-a-4' } })).toBe(0);
        expect(await em.count(model_1.Data, { where: { id: 'fork-a-5' } })).toBe(0);
        expect(await em.count(model_1.Data, { where: { id: 'fork-a-6' } })).toBe(0);
        // hot_block reflects the current view only: main-1, main-2, fork-b-3..7.
        const hotBlocks = (await em.query('SELECT height, hash FROM squid_processor.hot_block ORDER BY height')).map((r) => ({ height: r.height, hash: r.hash }));
...
//# sourceMappingURL=fork-cascade.test.js.map