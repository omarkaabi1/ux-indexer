...
describe('TypeormDatabase — data-type rollback fidelity', function () {
    ...
    async function runThenRollback(prevHead, blockHash, altHash, map) {
    ...
    describe('same-row multi-op within a single hot block', () => {
        it('insert → update → rollback leaves no trace', async () => {
            ...
            await runThenRollback({ height: -1, hash: '0x' }, 'h-0', 'h-0-alt', async (store) => {
                await store.insert(new model_1.Data({ id: 'x', text: 'a', integer: 1 }));
                await store.save(new model_1.Data({ id: 'x', text: 'b', integer: 2 }));
            });
            expect(await em.count(model_1.Data)).toBe(0);
        });
        it('insert → delete → rollback leaves no trace', async () => {
            await db.connect();
            const em = await (0, util_1.getEntityManager)();
            await runThenRollback({ height: -1, hash: '0x' }, 'h-0', 'h-0-alt', async (store) => {
                await store.insert(new model_1.Data({ id: 'x', text: 'a' }));
                await store.remove(model_1.Data, 'x');
            });
            expect(await em.count(model_1.Data)).toBe(0);
        });
        it('update → update → rollback restores the pre-block values', async () => {
            await db.connect();
            await db.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 10, hash: 'h-10' } }, async (store) => {
                await store.insert(new model_1.Data({ id: 'x', text: 'ORIG', integer: 100 }));
            });
            await runThenRollback({ height: 10, hash: 'h-10' }, 'h-11', 'h-11-alt', async (store) => {
                await store.save(new model_1.Data({ id: 'x', text: 'v1', integer: 1 }));
                await store.save(new model_1.Data({ id: 'x', text: 'v2', integer: 2 }));
            });
            const em = await (0, util_1.getEntityManager)();
            const row = await em.findOneOrFail(model_1.Data, { where: { id: 'x' } });
        ...
        it('delete → insert → rollback restores the original row', async () => {
            ...
            await runThenRollback({ height: 10, hash: 'h-10' }, 'h-11', 'h-11-alt', async (store) => {
                await store.remove(model_1.Data, 'x');
                await store.insert(new model_1.Data({ id: 'x', text: 'REPLACED', integer: 999 }));
            });
            const em = await (0, util_1.getEntityManager)();
            const row = await em.findOneOrFail(model_1.Data, { where: { id: 'x' } });
            expect(row.text).toBe('ORIG');
    ...
    describe('null / type rollback fidelity', () => {
        it('reverts non-null → null updates for every scalar and array type', async () => {
            await db.connect();
            const original = new model_1.Data({
                id: 'x',
                text: 'hello',
                textArray: ['a', 'b', 'c'],
                integer: 42,
            ...
            await runThenRollback({ height: 10, hash: 'h-10' }, 'h-11', 'h-11-alt', async (store) => {
                await store.save(new model_1.Data({
                    id: 'x',
                    text: null,
                    textArray: null,
                    integer: null,
        ...
        it('reverts a JSON transition object → array → null back to the original object', async () => {
            ...
            await db.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 10, hash: 'h-10' } }, async (store) => {
                await store.insert(new model_1.Data({ id: 'x', json: originalJson }));
            });
            await runThenRollback({ height: 10, hash: 'h-10' }, 'h-11', 'h-11-alt', async (store) => {
                await store.save(new model_1.Data({ id: 'x', json: [{ kind: 'array-now' }] }));
                await store.save(new model_1.Data({ id: 'x', json: null }));
            });
            const em = await (0, util_1.getEntityManager)();
            const row = await em.findOneOrFail(model_1.Data, { where: { id: 'x' } });
        ...
        it('reverts a zero-length bytea', async () => {
            await db.connect();
            await db.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 10, hash: 'h-10' } }, async (store) => {
                await store.insert(new model_1.Data({ id: 'x', bytes: new Uint8Array([]) }));
            });
            await runThenRollback({ height: 10, hash: 'h-10' }, 'h-11', 'h-11-alt', async (store) => {
                await store.save(new model_1.Data({ id: 'x', bytes: new Uint8Array([0xff, 0x00]) }));
            });
            const em = await (0, util_1.getEntityManager)();
            const row = await em.findOneOrFail(model_1.Data, { where: { id: 'x' } });
    ...
    describe('bulk entity operations', () => {
        it('rolls back an insert of >1000 rows in a single hot block', async () => {
            ...
            await runThenRollback({ height: -1, hash: '0x' }, 'h-0', 'h-0-alt', async (store) => {
                const rows = Array.from({ length: ROW_COUNT }, (_, i) => new model_1.Data({ id: `bulk-${i}`, integer: i }));
    ...
    describe('FK cascade limitations', () => {
        it('restores children when the parent was deleted via FK cascade', async () => {
            ...
            await db.transact({ prevHead: { height: -1, hash: '0x' }, nextHead: { height: 0, hash: 'h-0' } }, async (store) => {
                await store.insert(new model_1.Item('parent', 'P'));
                await store.insert(new model_1.Data({ id: 'child', text: 't', item: new model_1.Item('parent') }));
            });
            const em = await (0, util_1.getEntityManager)();
            expect(await em.count(model_1.Item)).toBe(1);
            expect(await em.count(model_1.Data)).toBe(1);
            ...
            await db.transactHot({
                ...
                await store.remove(model_1.Item, 'parent');
            });
            expect(await em.count(model_1.Item)).toBe(0);
            expect(await em.count(model_1.Data)).toBe(0); // cascade fired
            // Rollback.
            await db.transactHot({
                baseHead: { height: 0, hash: 'h-0' },
...
//# sourceMappingURL=data-type-rollback.test.js.map