...
class Server {
    ...
    async schema() {
        const schemaBuilder = await (0, dialect_1.getSchemaBuilder)({
            model: this.model(),
            subscriptions: this.options.subscriptions,
    ...
    async context() {
        ...
        if (await this.customResolvers()) {
            ...
            createOpenreader = () => {
                return new typeorm_1.TypeormOpenreaderContext(dbType, con, con, this.options.subscriptionPollInterval, this.options.log);
            };
        }
        else {
            ...
            createOpenreader = () => {
                return new db_1.PoolOpenreaderContext(dbType, pool, pool, this.options.subscriptionPollInterval, this.options.isolationLevel, this.options.log);
            };
        }
        return () => {
